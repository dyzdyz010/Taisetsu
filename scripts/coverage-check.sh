...
minimum_coverage="${LIFETIMER_CORE_COVERAGE_MINIMUM:-0.80}"
...
core_coverage=$(printf '%s' "${coverage_json}" | jq -r '
    [.targets[] | select(.name == "LifeTimerCore.framework") | .lineCoverage] | first // empty
')

if [[ -z "${core_coverage}" ]]; then
    echo "LifeTimerCore.framework coverage was not found." >&2
    exit 1
fi

printf 'LifeTimerCore line coverage: %.2f%% (minimum %.2f%%)\n' \
