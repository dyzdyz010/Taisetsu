# LifeTimer MVP Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build and publish a production-quality iOS/iPadOS LifeTimer MVP with Gregorian and Chinese-lunar anniversaries, recurrence, CRUD, categories and tags, local reminders, iCloud-ready persistence, Calendar export, three widget families, tests, and GitHub CI.

**Architecture:** A pure `LifeTimerCore` framework owns recurrence, occurrence, sorting, filtering, and widget snapshot value types. The app owns SwiftData models and repositories, then coordinates notification, EventKit, and App Group side effects only after successful persistence. The widget extension reads an atomic App Group JSON snapshot and never opens the app's SwiftData store.
...
LifeTimer/
  App/LifeTimerApp.swift                     Composition root and ModelContainer
...
LifeTimerWidget/
  LifeTimerWidgetBundle.swift                Widget bundle entry
  LifeTimerWidget.swift                      Configuration and provider
  LifeTimerWidgetEntry.swift                 Timeline entry
  LifeTimerWidgetView.swift                  Family-adaptive view
LifeTimerTests/                              Unit and service tests
LifeTimerUITests/                            Core user-flow UI tests
...
- Create: `LifeTimerCore/Configuration/AppConfiguration.swift`
- Modify: `LifeTimer.xcodeproj/project.pbxproj`
- Move: `LifeTimer/LifeTimerApp.swift` to `LifeTimer/App/LifeTimerApp.swift`
- Delete: `LifeTimer/Item.swift`
- Delete: `LifeTimer/ContentView.swift`
- Test: `LifeTimerTests/ProjectFoundationTests.swift`

**Interfaces:**
- Consumes: the existing Xcode template and approved design.
- Produces: schemes `LifeTimer`, `LifeTimerCore`, and `LifeTimerWidget`; an app group `group.com.dyz.LifeTimer`; an iCloud container `iCloud.com.dyz.LifeTimer`; `scripts/ci-test.sh` used by all later tasks and CI.
...
Run: `xcodebuild test -project LifeTimer.xcodeproj -scheme LifeTimer -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=26.5' -only-testing:LifeTimerTests/ProjectFoundationTests`

Expected: FAIL because `AppConfiguration` does not exist.

- [ ] **Step 4: Add XcodeGen configuration and the app configuration type**

Define four targets in `project.yml`: `LifeTimerCore` framework, `LifeTimer` application, `LifeTimerWidget` app extension, and the two test targets. Set iOS 18.0, Swift 6, strict concurrency, shared schemes, code coverage, app/widget entitlements, and filesystem source paths. Add this complete configuration type:

```swift
public enum AppConfiguration {
    public static let appGroupIdentifier = "group.com.dyz.LifeTimer"
    public static let cloudContainerIdentifier = "iCloud.com.dyz.LifeTimer"
    public static let widgetKind = "LifeTimerUpcoming"
}
```
...
Run: `xcodegen generate && xcrun swift-format lint -r LifeTimer LifeTimerCore LifeTimerWidget LifeTimerTests && bash scripts/ci-test.sh`
...
```bash
git add project.yml .swift-format scripts LifeTimer LifeTimerCore LifeTimerWidget LifeTimerTests LifeTimer.xcodeproj
...
**Files:**
- Create: `LifeTimerCore/Domain/CalendarKind.swift`
- Create: `LifeTimerCore/Domain/DisplayMode.swift`
- Create: `LifeTimerCore/Domain/RecurrenceRule.swift`
- Create: `LifeTimerCore/Domain/ReminderSpec.swift`
- Create: `LifeTimerCore/Domain/AnniversaryRecord.swift`
- Create: `LifeTimerCore/Domain/Occurrence.swift`
- Create: `LifeTimerCore/Domain/OccurrenceCalculator.swift`
...
Run: `bash scripts/ci-test.sh -only-testing:LifeTimerTests/OccurrenceCalculatorTests`
...
Run: `bash scripts/ci-test.sh -only-testing:LifeTimerTests/OccurrenceCalculatorTests && xcrun swift-format lint -r LifeTimerCore LifeTimerTests`
...
```bash
git add LifeTimerCore/Domain LifeTimerTests/OccurrenceCalculatorTests.swift
...
**Files:**
- Create: `LifeTimerCore/Domain/AnniversaryOrdering.swift`
- Create: `LifeTimerCore/Domain/AnniversaryFilter.swift`
- Create: `LifeTimerCore/Widget/WidgetSnapshot.swift`
- Test: `LifeTimerTests/AnniversaryOrderingTests.swift`
- Test: `LifeTimerTests/AnniversaryFilterTests.swift`
- Test: `LifeTimerTests/WidgetSelectionTests.swift`
...
Run: `bash scripts/ci-test.sh -only-testing:LifeTimerTests/AnniversaryOrderingTests -only-testing:LifeTimerTests/AnniversaryFilterTests`
...
Run: `bash scripts/ci-test.sh -only-testing:LifeTimerTests/AnniversaryOrderingTests -only-testing:LifeTimerTests/AnniversaryFilterTests -only-testing:LifeTimerTests/WidgetSelectionTests`

```bash
git add LifeTimerCore LifeTimerTests
...
**Files:**
- Create: `LifeTimer/Persistence/AnniversaryModel.swift`
- Create: `LifeTimer/Persistence/CategoryModel.swift`
- Create: `LifeTimer/Persistence/TagModel.swift`
- Create: `LifeTimer/Persistence/ReminderRuleModel.swift`
- Create: `LifeTimer/Persistence/ModelContainerFactory.swift`
- Create: `LifeTimer/Persistence/AnniversaryRepository.swift`
- Create: `LifeTimer/Persistence/DefaultCategorySeeder.swift`
- Create: `LifeTimer/Domain/AnniversaryDraft.swift`
- Test: `LifeTimerTests/AnniversaryRepositoryTests.swift`
- Test: `LifeTimerTests/DefaultCategorySeederTests.swift`
...
Run: `bash scripts/ci-test.sh -only-testing:LifeTimerTests/AnniversaryRepositoryTests`
...
Run: `bash scripts/ci-test.sh -only-testing:LifeTimerTests/AnniversaryRepositoryTests -only-testing:LifeTimerTests/DefaultCategorySeederTests`

```bash
git add LifeTimer/Persistence LifeTimer/Domain LifeTimerTests
...
**Files:**
- Create: `LifeTimer/App/AppDependencies.swift`
- Create: `LifeTimer/App/AppRootView.swift`
- Modify: `LifeTimer/App/LifeTimerApp.swift`
- Create: `LifeTimer/Features/Home/HomeView.swift`
- Create: `LifeTimer/Features/Home/HomeViewModel.swift`
- Create: `LifeTimer/Features/Home/AnniversaryHeroCard.swift`
- Create: `LifeTimer/Features/Home/AnniversaryRow.swift`
- Create: `LifeTimer/Features/Detail/AnniversaryDetailView.swift`
- Create: `LifeTimer/Shared/AnniversaryFormatters.swift`
- Create: `LifeTimer/Shared/CategoryStyle.swift`
- Test: `LifeTimerTests/HomeViewModelTests.swift`
...
Run: `bash scripts/ci-test.sh -only-testing:LifeTimerTests/HomeViewModelTests`
...
Run: `bash scripts/ci-test.sh -only-testing:LifeTimerTests/HomeViewModelTests && xcodebuild build -project LifeTimer.xcodeproj -scheme LifeTimer -destination 'generic/platform=iOS Simulator' CODE_SIGNING_ALLOWED=NO`

```bash
git add LifeTimer/App LifeTimer/Features/Home LifeTimer/Features/Detail LifeTimer/Shared LifeTimerTests
...
**Files:**
- Create: `LifeTimer/Features/Editor/AnniversaryEditorView.swift`
- Create: `LifeTimer/Features/Editor/AnniversaryEditorViewModel.swift`
- Create: `LifeTimer/Features/Editor/DateRuleSection.swift`
- Create: `LifeTimer/Features/Editor/RecurrenceSection.swift`
- Create: `LifeTimer/Features/Editor/ReminderSection.swift`
- Create: `LifeTimer/Features/Editor/CategoryTagSection.swift`
- Create: `LifeTimer/Features/Settings/CategoryManagerView.swift`
- Create: `LifeTimer/Features/Settings/TagManagerView.swift`
- Test: `LifeTimerTests/AnniversaryEditorViewModelTests.swift`
- Test: `LifeTimerTests/CategoryTagManagementTests.swift`
...
Run: `bash scripts/ci-test.sh -only-testing:LifeTimerTests/AnniversaryEditorViewModelTests`
...
Run: `bash scripts/ci-test.sh -only-testing:LifeTimerTests/AnniversaryEditorViewModelTests -only-testing:LifeTimerTests/CategoryTagManagementTests`

```bash
git add LifeTimer/Features/Editor LifeTimer/Features/Settings LifeTimerTests
...
**Files:**
- Create: `LifeTimer/Features/Calendar/CalendarView.swift`
- Create: `LifeTimer/Features/Calendar/CalendarViewModel.swift`
- Create: `LifeTimer/Features/Calendar/MonthGrid.swift`
- Create: `LifeTimer/Features/Settings/SettingsView.swift`
- Create: `LifeTimer/Features/Settings/PermissionStatusView.swift`
- Modify: `LifeTimer/App/AppRootView.swift`
- Test: `LifeTimerTests/CalendarViewModelTests.swift`
...
Run: `bash scripts/ci-test.sh -only-testing:LifeTimerTests/CalendarViewModelTests`
...
Run: `bash scripts/ci-test.sh -only-testing:LifeTimerTests/CalendarViewModelTests && xcodebuild build -project LifeTimer.xcodeproj -scheme LifeTimer -destination 'generic/platform=iOS Simulator' CODE_SIGNING_ALLOWED=NO`

```bash
git add LifeTimer/Features/Calendar LifeTimer/Features/Settings LifeTimer/App LifeTimerTests
...
**Files:**
- Create: `LifeTimer/Integrations/NotificationCenterClient.swift`
- Create: `LifeTimer/Integrations/ReminderScheduler.swift`
- Test: `LifeTimerTests/ReminderSchedulerTests.swift`
...
Run: `bash scripts/ci-test.sh -only-testing:LifeTimerTests/ReminderSchedulerTests`
...
```bash
git add LifeTimer/Integrations LifeTimer/Features/Editor LifeTimerTests
...
**Files:**
- Create: `LifeTimer/Integrations/EventStoreClient.swift`
- Create: `LifeTimer/Integrations/CalendarExportService.swift`
- Modify: `LifeTimer/Features/Detail/AnniversaryDetailView.swift`
- Modify: `LifeTimer/Info.plist`
- Test: `LifeTimerTests/CalendarExportServiceTests.swift`
...
Run: `bash scripts/ci-test.sh -only-testing:LifeTimerTests/CalendarExportServiceTests`
...
Run: `bash scripts/ci-test.sh -only-testing:LifeTimerTests/CalendarExportServiceTests && xcodebuild build -project LifeTimer.xcodeproj -scheme LifeTimer -destination 'generic/platform=iOS Simulator' CODE_SIGNING_ALLOWED=NO`

```bash
git add LifeTimer/Integrations LifeTimer/Features/Detail LifeTimer/Info.plist LifeTimerTests
...
**Files:**
- Create: `LifeTimer/Integrations/WidgetSnapshotStore.swift`
- Create: `LifeTimerWidget/LifeTimerWidgetBundle.swift`
- Create: `LifeTimerWidget/LifeTimerWidget.swift`
- Create: `LifeTimerWidget/LifeTimerWidgetEntry.swift`
- Create: `LifeTimerWidget/LifeTimerWidgetView.swift`
- Create: `LifeTimerWidget/LifeTimerWidget.entitlements`
- Modify: `project.yml`
- Modify: `LifeTimer.xcodeproj/project.pbxproj`
- Test: `LifeTimerTests/WidgetSnapshotStoreTests.swift`
- Test: `LifeTimerTests/WidgetTimelineTests.swift`
...
Run: `bash scripts/ci-test.sh -only-testing:LifeTimerTests/WidgetSnapshotStoreTests`
...
Use AppIntentConfiguration without custom grouping. The widget reads the snapshot only, uses `containerBackground`, marks title/date as privacy-sensitive, supports Dynamic Type, and links each rendered event to `lifetimer://anniversary/<uuid>`.

- [ ] **Step 6: Generate project, run widget tests/build, and commit**

Run: `xcodegen generate && bash scripts/ci-test.sh -only-testing:LifeTimerTests/WidgetSnapshotStoreTests -only-testing:LifeTimerTests/WidgetTimelineTests && xcodebuild build -project LifeTimer.xcodeproj -scheme LifeTimerWidget -destination 'generic/platform=iOS Simulator' CODE_SIGNING_ALLOWED=NO`

```bash
git add LifeTimer/Integrations LifeTimerWidget project.yml LifeTimer.xcodeproj LifeTimerTests
git commit -m "feat: add LifeTimer home screen widgets"
```

### Task 11: Reconciliation, Sync States, and Recovery

**Files:**
- Create: `LifeTimer/Coordination/ReconciliationCoordinator.swift`
- Create: `LifeTimer/App/StartupState.swift`
- Create: `LifeTimer/Features/Settings/SyncStatusView.swift`
- Modify: `LifeTimer/App/LifeTimerApp.swift`
- Modify: `LifeTimer/App/AppDependencies.swift`
- Test: `LifeTimerTests/ReconciliationCoordinatorTests.swift`
- Test: `LifeTimerTests/StartupRecoveryTests.swift`
...
Run: `bash scripts/ci-test.sh -only-testing:LifeTimerTests/ReconciliationCoordinatorTests`
...
Run: `bash scripts/ci-test.sh -only-testing:LifeTimerTests/ReconciliationCoordinatorTests -only-testing:LifeTimerTests/StartupRecoveryTests`

```bash
git add LifeTimer/Coordination LifeTimer/App LifeTimer/Features/Settings LifeTimerTests
...
- Modify: `LifeTimerWidget/*.swift`
- Replace: `LifeTimerUITests/LifeTimerUITests.swift`
- Replace: `LifeTimerUITests/LifeTimerUITestsLaunchTests.swift`
- Create: `LifeTimerTests/AccessibilityContractTests.swift`
- Create: `LifeTimerTests/PerformanceTests.swift`
- Create: `LifeTimer/Resources/Localizable.xcstrings`
...
Run: `bash scripts/ci-test.sh && LIFETIMER_UI_TESTS=1 bash scripts/ci-test.sh`
...
```bash
git add LifeTimer LifeTimerWidget LifeTimerTests LifeTimerUITests
git commit -m "test: cover LifeTimer user journeys"
```
...
```bash
xcodegen generate
git diff --exit-code -- LifeTimer.xcodeproj/project.pbxproj
xcrun swift-format lint -r LifeTimer LifeTimerCore LifeTimerWidget LifeTimerTests LifeTimerUITests
xcodebuild build -project LifeTimer.xcodeproj -scheme LifeTimer -destination 'generic/platform=iOS Simulator' CODE_SIGNING_ALLOWED=NO
...
Check every acceptance item in `docs/superpowers/specs/2026-08-03-lifetimer-design.md` against a concrete test, build artifact, or documented manual prerequisite. Fix any code-verifiable gap and repeat Step 1.

- [ ] **Step 3: Create the public GitHub repository**

Authenticate with `gh`, choose the available public name `LifeTimer` or the deterministic fallback `LifeTimer-iOS`, create without adding remote-generated files, add `origin`, and push the completed `main` branch.

```bash
gh repo create LifeTimer --public --source=. --remote=origin --push --description "A local-first iOS anniversary countdown app with lunar dates, reminders, iCloud sync, and widgets."
```
